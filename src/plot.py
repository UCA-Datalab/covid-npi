...
def plot_categorical_distribution(df: pd.DataFrame, col: str, title: str=None,
                                  dict_names: dict={}, sort_index:bool=False,
                                  top: int=100, figsize=(10,5)):
...
def plot_date_distribution(df: pd.DataFrame, col: str, title: str=None,
                          figsize: tuple=(10, 5)):
...
def plot_continuous_distribution(df: pd.DataFrame, col: str, title:str=None,
                                 bins: int=20, figsize: tuple=(10, 5), xlim=None):
...
def plot_categorical_distribution_by_date(df: pd.DataFrame, col: str, col_date: str,
                                          title: str="", dict_names: dict={},
                                          figsize: tuple=(10, 5), color_dict: dict=None):
    ...
    if color_dict is not None:
        ...
        color=None
    df_sub = (df_sub.groupby([df_sub[col_date].dt.year,
                     df_sub[col_date].dt.month, col])
              .size().unstack().rename(dict_names, axis=1))
...
def plot_date_difference(df: pd.DataFrame, col1: str, col2: str,
                        bins:int=20, title: str=None,
                         figsize: tuple=(10, 5), xlim=None):
    df = df.copy()
    df["NewCol"] = (df[col2] - df[col1]).dt.days
    if title is None:
        title = f"Días desde {col1} hasta {col2}"
    plot_continuous_distribution(df, "NewCol", bins=bins,
                                 title=title, figsize=figsize,
                                xlim=xlim)

    
def compare_df_dates(df1, df2, col,
                     label1="", label2="", title=None,
                     xlim=None,
                     figsize=(16, 5)):
